...
// Modified DLL implementation to support multi-remove
...
func (list *List[T]) Get(index int) (T, bool) {
	...
	// determine travel direction, last to first or first to last
...
// RangeRemove removes elements in the given range [start, end]
func (list *List[T]) RangeRemove(start, end int) {

	if !list.withinRange(start) {
		return
	}
	if list.size-1 < end {
		end = list.size - 1
	...
	var element *element[T]
	// determine traversal direction, last to first or first to last
	if list.size-start < start {
		element = list.last
		for e := list.size - 1; e != start; e, element = e-1, element.prev {
		}
	} else {
		element = list.first
		for e := 0; e != start; e, element = e+1, element.next {
		}
	}
